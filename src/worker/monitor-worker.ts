...
export async function executeWorkerPoll(options: { dryRun?: boolean; now?: Date } = {}) {
...
export async function startMonitorWorker() {
  ...
  do {
    ...
    console.log(`[worker] run started at ${startedAt.toISOString()} provider=${env.MONITOR_ACTIVITY_PROVIDER}`);
    const resolved = {
    ...
    try {
      const { summary } = await executeWorkerPoll({ dryRun });
